...
需要安装 `.ccx` 插件的用户，请从仓库内的 [`releases/`](releases/) 文件夹下载对应插件与版本的安装包；`plugins/` 目录主要用于查看源码。
...
| Color Layer Batch Merge / 调色图层批量合并 | `plugins/color-layer-batch-merge/` | `0.3.0` | 记忆调色层与目标层，在安全文档副本中递归处理嵌套组、剪贴蒙版和锁定层，并逐层完成批量合并。 |
| psdautosave / PSD 自动备份 | `plugins/psdautosave/` | `0.1.1-test` | 绑定一个 PSD/PSB 目标文件，按手动或定时间隔保存本地备份副本，并显示运行计时与最近备份状态。 |
...
To install `.ccx` plugins, download the installer for the matching plugin and version from the repository [`releases/`](releases/) folder. The `plugins/` directory is mainly for source code.
...
| Color Layer Batch Merge | `plugins/color-layer-batch-merge/` | `0.3.0` | Applies saved color layers to nested target layers through ordered clipping-mask merges in a protected document duplicate. |
| psdautosave | `plugins/psdautosave/` | `0.1.1-test` | Binds one PSD/PSB target and saves guarded local backup copies manually or on a timer, with elapsed runtime and latest-backup status. |
